...
async data, via the official HN Firebase API) and, for each one, uses
Claude to infer the industry it's most relevant to and generate a
...
- `server.js` fetches the top 5 story IDs from `https://hacker-news.firebaseio.com/v0/topstories.json`,
  ...
  concurrently (independent websites, no shared rate limit). Claude calls
...
- `index.html` reads that stream and appends each story's card to the page
  ...
  Claude fails to generate a summary for a story, that story instead shows a
  clear "Claude failed to generate a summary" message with a direct link to
...
under Settings → API Keys) for the summarization step. Set it as an
environment variable — never commit it or paste it into a file in this repo.
